...
class ALDA_LOSS(BaseModule):

    def __init__(self, threshold=0.9, init_cfg=None):
        super().__init__(init_cfg)
        self.threshold = threshold

    def forward(self, ad_out_score, labels_source, softmax_out):
        """
        ...
        labels_source_mask = torch.zeros(batch_size,
                                         class_num).to(ad_out.device).scatter_(
                                             1, labels_source.unsqueeze(1), 1)
        ...
        preds_source_mask = torch.zeros(batch_size,
                                        class_num).to(ad_out.device).scatter_(
                                            1, argpred.unsqueeze(1), 1)
        maxpred, argpred = torch.max(probs_target, dim=1)
        preds_target_mask = torch.zeros(batch_size,
                                        class_num).to(ad_out.device).scatter_(
                                            1, argpred.unsqueeze(1), 1)

        # filter out those low confidence samples
        target_mask = (maxpred > self.threshold)
        preds_target_mask = torch.where(target_mask.unsqueeze(1),
                                        preds_target_mask,
                                        torch.zeros(1).to(ad_out.device))
        # construct the confusion matrix from ad_out. See the paper for
        # more details.
        ...
        loss_pred = torch.mul(confusion_matrix,
                              preds_mask.unsqueeze(1)).sum(dim=2)
        ...
        # reg_loss
        reg_loss = nn.CrossEntropyLoss()(ad_out_score[:batch_size],
                                         labels_source)

        # corrected target loss function
        target_probs = 1.0 * softmax_out[batch_size:]
        correct_target = torch.mul(confusion_matrix.detach()[batch_size:],
                                   preds_target_mask.unsqueeze(1)).sum(dim=2)
